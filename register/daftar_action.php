...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
	...
	mysqli_stmt_bind_param($querySimpan, "sssissssssi", $kodePeserta, $email, $passwordHashed, $nisn, $jurusan, $nama, $gender, $birthday, $alamat, $asalSekolah, $uas);

	if (mysqli_stmt_execute($querySimpan)) {
		$idNilai = generateUuid();
		$querynilai = mysqli_prepare($mysqli, "INSERT INTO nilai (id, no_pendaftaran, C1, C2, C3, C4, C5) VALUES (?, ?, ?, 0, 0, 0, 0)");
		mysqli_stmt_bind_param($querynilai, "ssi", $idNilai, $kodePeserta, $uas);
		mysqli_stmt_execute($querynilai);
		mysqli_stmt_close($querynilai);

		$idNormalisasi = generateUuid();
		$querynormalisasi = mysqli_prepare($mysqli, "INSERT INTO normalisasi (id, no_pendaftaran, C1, C2, C3, C4, C5) VALUES (?, ?, 0, 0, 0, 0, 0)");
		mysqli_stmt_bind_param($querynormalisasi, "ss", $idNormalisasi, $kodePeserta);
		mysqli_stmt_execute($querynormalisasi);
		mysqli_stmt_close($querynormalisasi);

		$_SESSION['flash_message'] = 'Registrasi Akun Berhasil, Silahkan Login!';
		header("Location: " . BASE_URL . "/login");
	} else {
		$_SESSION['flash_message'] = 'Registrasi Akun Gagal, Silahkan Coba Lagi!';
